...
| Export briefs are exports, never the store, v0.19.0 | Two new brief inputs — `--brief --grep X` (thematic memory) and `--brief` over a `conversations.json` (claude.ai/ChatGPT history via `parse_web_export_all`, one cached note per conversation, uuids as citations) — both REQUIRE an explicit `-o` and never read or write the project's standing brief: a thematic subset grafting the full distillation (or hooks refreshing over a filtered timeline) would silently corrupt the memory. Selection flags compose in-memory there, nothing sticky — there is no stamp store to stick to. |
| `--keep since:` time windows, v0.19.0 | `since:7d` (or an ISO date) keeps sessions by LAST activity — a middle-aged session touched yesterday is recent. Reuses `_since_cutoff` with `last_ts=None` (relative to now), so the sticky stamp spec re-derives the cutoff at every refresh: the time window slides exactly like the count window. Union semantics with `first:`/`last:`, chronological, deduped. |
| Composability audited both ways, v0.19.0 | Documenting "--llm composes with every selection/filter flag" (true — one funnel: resolve → slice → build_document) triggered the reverse audit. Found: `--brief -o clipboard` wrote a literal file named `clipboard` (now copies — WITH the grafted distillation, because clipboard is *shipping* the memory, not exporting a skeleton), and `--brief --format json` was silently ignored (now a loud error). Remaining non-compositions are deliberate and documented: `--fit`×`--llm` errors, `--exclude`/`--keep` are brief-only. Composition candidates parked on the README roadmap: brief over web exports, `--brief --grep` thematic memory, `--keep since:7d`. |
...
| `--exclude` picker edits, never replaces, v0.20.0 | Bare `--exclude` used to start from scratch: whatever you picked REPLACED the stamp's stored set, so keeping an old exclusion meant re-picking it every time (field request, 2026-08-27). The picker now receives the stored prefixes, marks matching sessions ✗, and applies typed numbers as a symmetric-difference TOGGLE (`excluded ^ picked`) — one mechanism both adds and removes. Empty input keeps the current set (with nothing stored that is the old behavior, so zero breaking change); `none` inside the picker mirrors `--exclude none`. Stale stored prefixes fall away naturally: they pre-select nothing, so the toggle result no longer carries them. |
| Code-map knowledge lines + in-project brief mirrors, v0.20.0 | Structure alone ("6 communities") answers less than the graph knows, so `_code_bridges` names the strongest cross-community links (endpoints, relation, both communities — where subsystems touch; ranked by endpoint-degree sum) and `_code_flows` lists graphify's labeled hyperedges — both capped at 3, both computed from `graph.json` directly rather than parsing GRAPH_REPORT.md (report formats drift; node-link JSON is the stable contract). Mirrors: the brief also lives in the project — `raw/project-memory.md` (graph corpus) and root `BRIEF.md` (human/git copy) refresh on every STORE write, explicit or SessionEnd hook, via `sync_brief_mirrors`. Strictly refresh-only to honor "hooks never create files": creating the file once (`-o graphify` / `touch BRIEF.md`) is the opt-in, deleting it the opt-out. `update_brief_skeleton` now returns the written text so the hook can hand it to the sync. Caveat, documented: with BOTH mirrors present a graphify scan ingests the memory twice — raw/ is for the graph, BRIEF.md for humans. |
| graphify bridge (`-o graphify` + `## Code map`), v0.20.0 | Both directions, zero config, tools stay decoupled. Outbound: `-o graphify` files the redacted doc into `raw/` (graphify's own ingest convention) with the frontmatter graphify maps onto nodes (`captured_at`/`source_url`/`contributor`); the brief is ONE evolving `project-memory.md` (overwrite — git owns history; per-refresh copies would breed duplicate nodes), handoffs are per-session; never shells out to graphify (zero deps — the `/graphify --update` next step is printed), warns when `raw/` isn't gitignored (memory in a repo is egress), and `--brief --grep` refuses the target so a filtered view can't clobber the standing corpus doc. Inbound: `_code_map` reads `graphify-out/graph.json` (networkx node-link) + `.graphify_labels.json` and appends a capped section to the deterministic skeleton — counts, top communities with hub concepts, and `built_at_commit` measured against the reflog for a commits-behind note; tolerant like the git helpers (any surprise → no section), so non-graphify users see zero change. |
| Shipped Claude Code skill (`--install-skill`), v0.20.0 | An agent driving chf without documentation invents subcommands (`chf brief …`), picks the wrong provider (`claude` when the user meant the subscription-backed `claude-cli`) and the wrong session (bare `chf` inside a live session exports the CURRENT one) — measured with a baseline subagent before the skill was written; a rerun with the skill produced correct commands on every scenario. The content deliberately lives twice: `skills/claude-handoff/SKILL.md` (canonical, browsable) and the `SKILL_MD` constant in integrations.py (what pip/brew/curl installs carry — a package data file would break the single-file build), pinned byte-identical by a test, same pattern as the generated single file itself. Installer edits are surgical and test-pinned: the CLAUDE.md trigger section is appended only when no `# claude-handoff` heading exists (hand-written ones respected), removal deletes heading→next-heading and round-trips the rest byte-for-byte, and the skill dir is pruned only when empty. |
